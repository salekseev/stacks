...
- [ ] **Step 1: Revoke the exposed provider API key**

The redacted provider API key appears twice in `/mnt/spool/apps/config/hindsight/litellm/config.yaml` (on `openai/gpt-5.4-mini` and `openai/text-embedding-3-small`). Revoke it in the owning OpenAI account. A commented-out corporate AI-gateway base URL beside it indicates the credential may be employer-issued, which makes this mandatory.
...
```bash
cd /home/salekseev/src/github.com/salekseev/stacks
git log --all -S "provider-api-key-prefix" --oneline | head
