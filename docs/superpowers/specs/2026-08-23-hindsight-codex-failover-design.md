...
- `router_settings.fallbacks: [{hindsight-default: [gpt-5.4-mini]}]`, where `gpt-5.4-mini` is declared **twice** under one `model_name` — `github_copilot/gpt-5.4-mini` and `openai/gpt-5.4-mini` with a hardcoded provider API key (redacted) — so litellm load-balances across Copilot and a direct provider API account.
...
- The redacted provider API key, hardcoded twice in `config.yaml` (`openai/gpt-5.4-mini` and `openai/text-embedding-3-small`). A commented-out corporate AI-gateway base URL beside it suggests an employer-issued credential, which makes rotation mandatory rather than discretionary.
